...
/**
 * FASE-PLANTAO-MULTIPOSTO-WORKSPACE-1 — painel de saúde de UM posto/função
 * (§16/§17/§33 da fase). Genérico por construção: recebe `rotulo` (nunca
 * hardcoda "DBA"/"Linux"/...) e o resultado já calculado de
 * `avaliarSaudePlantao()` — nunca recalcula nada aqui, só apresenta. Não
 * criar `CardDBA.tsx`/`CardLinux.tsx`/etc. — este componente serve
 * qualquer função de qualquer Grupo de Plantão multi-função presente ou
 * futuro (`grupo.funcoesEsperadas`).
 *
...
export function CardFuncaoPlantao({ rotulo, saude, selecionado, onSelecionar }: CardFuncaoPlantaoProps) {
  ...
  return (
    <button
      type="button"
      className={`overview-operation-card ${varianteStatus}${selecionado ? ' selected' : ''}`}
